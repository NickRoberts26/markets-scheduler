            ...
            {marketplaces?.map((marketplace) => {
                return (
                    <Link key={marketplace.uid} href={`/local-markets/${marketplace.marketplaceName.replace(/\s+/g, '-').toLowerCase()}`} className='relative h-[35vh] lg:h-[40vh] mb-5 transition-all duration-500 hover:scale-105'>
                        ...
                        <div className='absolute top-0 left-0 h-full w-full text-white flex text-center items-center justify-center text-3xl'>{marketplace.marketplaceName}</div>
                        {!hasError && (
                            <Image
                                ...
                                width={1000}
                                height={1000}
                                className='border-4 border-green-500 rounded-3xl object-cover h-[35vh] lg:h-[40vh]'
